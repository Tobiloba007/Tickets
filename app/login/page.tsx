...
import { LoginAccount } from '@/redux/features/authentication/AuthActions';
import { useDispatch } from 'react-redux';
import { PiEye, PiEyeSlash } from "react-icons/pi";
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import Image from 'next/image';
import { LoginParams } from '@/types/app';
  ...
  const values:LoginParams = {email: email, password: password}

  const dispatch = useDispatch();

  const handleSubmit = () => {
    dispatch(LoginAccount(values, setError, setLoading, router.push))
  }
  ...
  return (
    <div className='flex flex-col items-center justify-start w-full px-5 py-6 max-w-screen-2xl mx-auto font-outfit md:px-10 lg:px-12 xl:px-20'>
      ...
      className='flex items-center justify-between w-full xl:mt-6'>
          <Image width={112} height={322}  className='w-[112px] h-322px] lg:w-[160px] lg:h-[40px] xl:w-[168px] xl:h-[48px]'
        ...
        <div className='flex flex-col items-center justify-start w-full mt-3'>
          ...
          <div className='flex flex-col items-start justify-start w-full mt-6'>
              ...
              <div className='flex items-center justify-between w-full mt-7 xl:mt-8'>
                 ...
                 <Link href='/forgotPassword' className='text-xs font-medium text-[#2A1D34] ml-3 xl:text-sm xl:ml-4'>
                      Forgot password?
                 </Link>
            </div>
